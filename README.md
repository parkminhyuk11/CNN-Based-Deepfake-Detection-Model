...
본 프로젝트는 CNN 기반 이미지 분류 모델을 활용해 Real 얼굴 이미지와 AI 기반 얼굴 조작 이미지(Fake)를 구분하는 딥페이크 탐지 모델임.

최종 모델은 **EfficientNet-B4**를 backbone으로 사용함.
원본 RGB 이미지와 Artifact Map 이미지를 각각 학습한 뒤, 두 모델의 Fake probability를 평균하는 **RGB + Artifact Map Ensemble** 방식을 적용함.

---

## 1. 프로젝트 개요

딥페이크 기술의 품질이 높아지면서 가짜 뉴스, 얼굴 도용, 명예훼손, 사생활 침해와 같은 문제가 증가하고 있음.
특히 최근 생성·편집된 얼굴 이미지는 사람 눈으로 Real/Fake를 구분하기 어려운 경우가 많아 이미지 기반 자동 탐지 모델의 필요성이 커지고 있음.
...
본 프로젝트의 Fake 데이터는 모든 유형의 딥페이크를 포함한다기보다, **AI 기반 표정 조작 및 얼굴 편집 이미지**를 중심으로 구성됨.
따라서 본 연구는 넓은 의미의 **Face Manipulation Detection**으로 해석 가능함.

---

## 2. 데이터셋 구성

본 프로젝트에서는 Real 이미지 500장, Fake 이미지 500장으로 총 1000장의 이미지 데이터셋을 구성함.
...
Train과 Validation은 8:2 비율로 분할함.
...
이미지 데이터셋은 용량 및 저작권 문제로 GitHub에는 포함하지 않음.
실행 전 위 폴더 구조에 맞게 데이터를 배치해야 함.
...
RGB 모델은 원본 얼굴 이미지를 입력으로 사용함.

```text
Original RGB Image → EfficientNet-B4 → Real / Fake
```

RGB 이미지는 얼굴의 전체적인 구조, 색상, 조명 정보를 포함하고 있어 기본적인 Real/Fake 분류에 사용됨.

---

### 3.2 Artifact Map 기반 탐지

Artifact Map은 원본 이미지와 Gaussian Blur 이미지의 차이를 계산하여 생성함.

```text
Artifact Map = | Original Image - Gaussian Blurred Image |
```

Gaussian Blur를 적용하면 얼굴의 전체 구조, 색상, 조명과 같은 저주파 정보는 남고, 피부 질감, 경계선, 노이즈와 같은 고주파 정보는 약해짐.

따라서 원본 이미지에서 Gaussian Blur 이미지를 빼면 고주파 성분이 강조된 Artifact Map을 얻을 수 있음.

```text
Original Image → Gaussian Blur → Difference Image → Artifact Map
```

Artifact Map은 사람이 직접 Fake 여부를 판별하기 위한 이미지가 아니라, CNN 모델이 미세한 질감, 경계선, 노이즈 패턴을 보조적으로 학습하도록 만든 입력임.

**[이미지 삽입 위치]**
PPT의 `Artifact Map 생성 원리` 이미지 삽입 권장.

```markdown
![Artifact Map Generation](assets/artifact_map_generation.png)
```
...
RGB 모델과 Artifact Map 모델은 각각 따로 학습함.

이후 두 모델의 Fake probability를 평균하여 최종 Fake probability를 계산함.

```text
Final Fake Probability = (RGB Fake Probability + Artifact Fake Probability) / 2
```

최종 판정 기준은 threshold 0.5임.
...
앙상블을 사용한 이유는 RGB 이미지와 Artifact Map이 서로 다른 정보를 담고 있기 때문임.
...
**[그림 삽입 위치]**
PPT의 `RGB Image + Artifact Map → EfficientNet-B4 → Real/Fake` 흐름도 또는 제안 방법 구조 그림 삽입 권장.

```markdown
![Proposed Ensemble Architecture](assets/proposed_ensemble_architecture.png)
```
...
본 프로젝트에서는 다음 4개의 모델을 비교함.

| Model             | 선정 이유                    |
| ----------------- | ------------------------ |
| EfficientNet-B4   | 최종 후보 모델, 성능 중심 CNN      |
| MobileNetV3-Large | 경량 CNN 모델 비교             |
| ResNet-50         | 전통적인 CNN baseline        |
| ViT-B/16          | Transformer 기반 이미지 모델 비교 |

EfficientNet-B4는 RGB 입력과 Artifact Map 입력 모두에서 안정적인 성능을 보여 최종 모델로 선정함.

MobileNetV3는 경량 모델로서 높은 성능을 보였지만, 본 프로젝트에서는 속도보다 최종 정확도와 입력별 안정성을 더 중요하게 판단함.

ResNet-50과 ViT-B/16은 이번 데이터셋 및 학습 조건에서는 성능이 낮게 나타남.
특히 ViT-B/16은 모든 이미지를 Fake로 예측하는 클래스 쏠림 현상이 발생하여 추가 검증이 필요한 결과로 해석함.

**[표 또는 그래프 삽입 위치]**
PPT의 `모델 선정 과정 - 224×224 비교 결과` 표/그래프 삽입 권장.

```markdown
![EfficientNet 224 Comparison](assets/efficientnet_224_comparison.png)
```

**[표 또는 그래프 삽입 위치]**
PPT의 `모델 선정 과정 - 권장 해상도 적용 결과` 표/그래프 삽입 권장.

```markdown
![EfficientNet Recommended Resolution](assets/efficientnet_recommended_resolution.png)
```
...
```bash
pip install torch torchvision pytorch-lightning numpy pillow pyyaml
```
...
### 6.3 Artifact Map 모델 학습

Artifact Map 모델을 학습할 때는 `config.yaml`을 다음과 같이 설정함.
...
**[그래프 삽입 위치]**
PPT의 `RGB / Artifact 단일 입력 성능 비교 막대그래프` 삽입 권장.

```markdown
![RGB Artifact Single Input Result](assets/rgb_artifact_single_input_result.png)
```
